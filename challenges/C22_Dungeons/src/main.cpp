...
void intMethod() {
    ...
    int w, h, comp;
    const stbi_uc* in_encoded_picture = stbi_load("data/dungeons_portal_enc.png", &w, &h, &comp, 0); // image encodé (char*)
    stbi_uc* reference_picture = new stbi_uc[w * h];    // image de référence
    const stbi_uc* reference_encoded_picture_no_xor = reference_picture;

    // Performing int power operations and filling array
    for (int y = 0; y < h; ++y) {
        for (int x = 0; x < w; ++x)
        {
    ...
    for (int key = 0; key < KEY_BRUTEFORCE_COUNT; ++key) {
        std::string fileName = "data_out/decoded" + std::to_string(key) + ".bmp";
